...
describe('Test des endpoints /bookings', function() {
	...
	describe('consultation de reservation, GET /reservations/{id}', function() {
		it('si la reservation n\'existe pas, on reçoit un 404', function() {
			return request
				.get('/bookings/LPDW-VOL')
				.set('authentification', 'MY_TOKEN')
				.expect(404);
		});
		...
		it('si la requete ne content pas les identifiants de l\'utilisateur, on reçoit un 401', function() {
			return request
				.get('/bookings/LPDW-VOL-1')
				.expect(401);
		});

		it('si la reservation existe et appartient à l\'utilisateur, on reçoit un 200', function() {
			return request
				.get('/bookings/LPDW-VOL-1')
				.set('authentification', 'MY_TOKEN')
				.expect(200)
